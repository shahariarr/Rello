    ...
    <aside id="sidebar-wrapper">
        ...
        <ul class="sidebar-menu">
            ...
            @can(['index-banner', 'index-service', 'index-property', 'index-rent'])
                ...
                <li
                    class="dropdown {{ Route::is('banner.*') || Route::is('service.*') || Route::is('properties.*') || Route::is('rents.*') ? 'active' : '' }}">
                    ...
                    @endcan --}}
                        @can('index-banner')
                            <li class="{{ Route::is('banners.index') ? 'active' : '' }}"><a class="nav-link"
                                    href="{{ route('banner.edit') }}">Banner List</a></li>
                        @endcan
                        {{-- @can('create-service')
                    <li class="{{ Route::is('services.create') ? 'active' : '' }}"><a class="nav-link" href="{{ route('service.create') }}">Create Service</a></li>
                    @endcan --}}
                        @can('index-service')
                            <li class="{{ Route::is('services.index') ? 'active' : '' }}"><a class="nav-link"
                                    href="{{ route('service.edit') }}">Service List</a></li>
                        @endcan
                        @can('index-property')
                            <li class="{{ Route::is('properties.index') ? 'active' : '' }}"><a class="nav-link"
                                    href="{{ route('properties.index') }}">Property List</a></li>
                        @endcan
                        @can('index-rent')
                            <li class="{{ Route::is('rents.index') ? 'active' : '' }}"><a class="nav-link"
                                    href="{{ route('rents.index') }}">Rent List</a></li>
                        @endcan
                    </ul>
                </li>
            @endcan

            @can(['index-about', 'index-contact'])
                <li class="menu-header">Other Management</li>
                <li class="dropdown {{ Route::is('about.*') || Route::is('contact.*') ? 'active' : '' }}">
                    <a href="#" class="nav-link has-dropdown" data-toggle="dropdown"><i
                            class="fas fa-info-circle"></i> <span>Other Management</span></a>
                    <ul class="dropdown-menu">
                        @can('index-about')
                            <li class="{{ Route::is('about.edit') ? 'active' : '' }}"><a class="nav-link"
                                    href="{{ route('about.edit') }}">About</a></li>
                        @endcan
                        @can('index-contact')
                            <li class="{{ Route::is('contactdata.edit') ? 'active' : '' }}"><a class="nav-link"
                                    href="{{ route('contactdata.edit') }}">Contact</a></li>
                        @endcan
                    </ul>
                </li>
            @endcan
